...
var themeName = "tajam";
var projectPath = "./wp-content/themes/" + themeName;

gulp.task("default", function() {
  console.log("Task sequence is running...");
  console.log(
    "🌅 Run " + color('"gulp images"', "GREEN") + " to optimaze img size."
  );
  console.log(color("---------------------------", "MAGENTA"));
  console.log(color("‼️ ", "RED") + " BEFORE DEPLOY RUN:");
  console.log(
    "1. " + color('"gulp unique-color"', "YELLOW") + " - changes color shades;"
  );
  console.log(
    "2. " + color('"gulp prod"', "YELLOW") + " - changes classnames to hash;"
  );
  console.log(color("---------------------------", "MAGENTA"));
  console.log("To run dev - " + color('"gulp"', "GREEN"));
  console.log(color("---------------------------", "MAGENTA"));
  if (fs.existsSync("./hashMap.json")) {
    runSequence("sass", "template-to-dev", "babelify", [
      "sass:watch",
      "babelify:watch"
    ]);
  } else {
    runSequence("sass", "babelify", ["sass:watch", "babelify:watch"]);
  }
});

gulp.task("sass", function() {
  return gulp
    .src(projectPath + "/scss/*.scss")
...
function randprefix() {
  ...
  return "[hash]"; // [hash] - variable replaced with generated hash
}
var opts = {
  hashType: "md5",
  digestType: "base32",
  ...
  classnameFormat: "_" + randprefix(), // generates random prefix to avoid classnames whitch start with number
  type: ".json"
};
var processors = [require("postcss-hash-classname")(opts)];

gulp.task("style-to-prod", function() {
  return gulp
    .src([projectPath + "/style.css"])
...
gulp.task("template-to-prod", function(cb) {
  ...
  gulp
    .src([projectPath + "/*.php", projectPath + "/js/*.js"])
